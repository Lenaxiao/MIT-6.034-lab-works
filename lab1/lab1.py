...
brother = IF( 
  AND( 'male (?x)','parent (?z) (?x)','parent (?z) (?y)', NOT( 'same-identity (?x) (?y)') ), 
  THEN( 'brother (?x) (?y)' ) 
)
sister = IF(
  AND( 'female (?x)','parent (?z) (?x)','parent (?z) (?y)', NOT('same-identity (?x) (?y)') ),
  THEN( 'sister (?x) (?y)' )
)
mother = IF(
...
cousin = IF(
  AND('parent (?x) (?a)', 'parent (?y) (?b)', NOT('same-identity (?x) (?y)'), OR('brother (?x) (?y)', 'sister (?x) (?y)')),
  THEN('cousin (?a) (?b)')
)
grandparent = IF(
...
family_rules = [same_identity, brother, sister, mother, father, son, daughter, cousin, grandparent, grandchild]
...
WHAT_I_FOUND_INTERESTING = 'a way to practice backtracking'
